...
router.get(
  ...
  [
    ...
    query("status")
      .optional()
      .isIn(["active", "completed", "closed", "defaulted"]),
...
export const createInvestment = async (req, res) => {
  ...
  if (investor.status !== "active") {
    return res
      .status(400)
      .json({ message: "Cannot create investment for inactive investor" });
  }
  ...
  await investment.populate([
    ...
    {
      path: "plan",
      select: "planId name paymentType interestType interestRate tenure",
    },
    ...
    if (status && status !== oldStatus) {
      ...
      if (investor) {
        if (oldStatus === "active" && status !== "active") {
          investor.activeInvestments = Math.max(
            0,
    ...
    res.json({
      ...
      data: {
        ...
        documentsByCategory: {
          ...
          payment_proof:
            investment.getDocumentsByCategory("payment_proof").length,
          communication:
            investment.getDocumentsByCategory("communication").length,
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const sortedTimeline = investment.timeline.sort(
...
router.post(
  ...
  [
    body("type")
      ...
      .withMessage("Invalid timeline entry type"),
    body("description")
      .trim()
      .notEmpty()
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const investment = await Investment.findOne(query)
      .select(
        "investmentId schedule totalExpectedReturns totalPaidAmount remainingAmount"
      )
  ...
  asyncHandler(async (req, res) => {
    ...
    res.json({
      ...
      data: {
        ...
        remainingValue:
          (totalValue[0]?.total || 0) - (totalPaid[0]?.total || 0),
