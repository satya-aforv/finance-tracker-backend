...
router.get(
  ...
  [
    ...
    query("type")
      .optional()
      .isIn(["interest", "principal", "mixed", "penalty", "bonus"]),
    query("investment").optional().isMongoId(),
    query("investor").optional().isMongoId(),
    query("isVerified").optional().isBoolean().toBoolean(),
    query("hasDocuments").optional().isBoolean().toBoolean(),
    query("dateFrom").optional().isISO8601(),
    query("dateTo").optional().isISO8601(),
    query("minAmount").optional().isFloat({ min: 0 }),
    query("maxAmount").optional().isFloat({ min: 0 }),
    query("minInterest").optional().isFloat({ min: 0 }),
    query("maxInterest").optional().isFloat({ min: 0 }),
    query("minPrincipal").optional().isFloat({ min: 0 }),
    query("maxPrincipal").optional().isFloat({ min: 0 }),
  ],
  asyncHandler(async (req, res) => {
    ...
    if (typeof isVerified === "boolean") {
      query.verifiedBy = isVerified ? { $ne: null } : null;
    }

    if (typeof hasDocuments === "boolean") {
      query["documents.0"] = hasDocuments
        ? { $exists: true }
        : { $exists: false };
    }
    ...
    if (minInterest || maxInterest) {
      query.interestAmount = {};
      if (minInterest) query.interestAmount.$gte = parseFloat(minInterest);
      if (maxInterest) query.interestAmount.$lte = parseFloat(maxInterest);
    }

    if (minPrincipal || maxPrincipal) {
      query.principalAmount = {};
      if (minPrincipal) query.principalAmount.$gte = parseFloat(minPrincipal);
      if (maxPrincipal) query.principalAmount.$lte = parseFloat(maxPrincipal);
    }

    // Role-based access: investors only see their payments
    if (req.user.role === "investor") {
      const investorDoc = await Investor.findOne({ userId: req.user._id });
      if (investorDoc) {
        query.investor = investorDoc._id;
      } else {
        query.investor = null; // No payments if investor profile not found
      }
    }
...
router.post(
  ...
  [
    ...
    body("scheduleMonth")
      .isInt({ min: 1 })
      .withMessage("Schedule month must be a positive integer"),
    body("amount")
      .isFloat({ min: 0.01 })
    ...
    body("interestAmount")
      .optional()
      .isFloat({ min: 0 })
      .withMessage("Interest amount must be non-negative"),
    body("principalAmount")
      .optional()
      .isFloat({ min: 0 })
      .withMessage("Principal amount must be non-negative"),
    body("penaltyAmount")
      .optional()
      .isFloat({ min: 0 })
  ...
  asyncHandler(async (req, res) => {
    ...
    if (investment.status !== "active") {
      return res
        .status(400)
        .json({ message: "Cannot record payment for non-active investment" });
    }

    // Debug: Log the schedule and requested month
    console.log(
      "Investment schedule:",
      investment.schedule.map((s) => ({ month: s.month, status: s.status }))
    );
    console.log("Requested schedule month:", scheduleMonthNum);

    // Find schedule item - be more flexible with status checking
    const scheduleItem = investment.schedule.find(
      (s) => s.month === scheduleMonthNum
    );
    if (!scheduleItem) {
      return res.status(400).json({
        ...
        debug: {
          ...
          scheduleMonths: investment.schedule.map((s) => ({
            month: s.month,
            type: typeof s.month,
            status: s.status,
          })),
    ...
    if (!interestAmount && !principalAmount && !penaltyAmount && !bonusAmount) {
      const remainingAmount = Math.max(
        0,
        scheduleItem.totalAmount - scheduleItem.paidAmount
      );
      const remainingInterest = Math.max(
        0,
        scheduleItem.interestAmount -
          Math.min(scheduleItem.paidAmount, scheduleItem.interestAmount)
      );

      finalInterestAmount = Math.min(parseFloat(amount), remainingInterest);
      finalPrincipalAmount = Math.max(
        0,
        parseFloat(amount) - finalInterestAmount
      );
    }
    ...
    try {
      ...
      scheduleItem.paidAmount =
        (scheduleItem.paidAmount || 0) + parseFloat(amount);
      ...
      if (documents.length > 0)
        timelineDescription += ` with ${documents.length} document(s)`;
      ...
      if (documents.length > 0) {
        await investment.addTimelineEntry(
          "document_uploaded",
          `Payment documents uploaded: ${documents
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    const { status, verifiedBy, notes, referenceNumber, paymentMethod } =
      req.body;
    ...
    if (investment) {
      ...
      if (referenceNumber !== undefined && referenceNumber !== oldReference) {
        changes.push(
          `reference: ${oldReference || "none"} → ${referenceNumber || "none"}`
        );
      }
      ...
      if (changes.length > 0) {
        const changeDescription = `Payment ${
          payment.paymentId
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (investment) {
      await investment.addTimelineEntry(
        "document_uploaded",
        `Additional payment documents uploaded for ${
          payment.paymentId
        }: ${newDocuments.map((d) => d.originalName).join(", ")}`,
        req.user._id,
        0,
        {
          paymentId: payment.paymentId,
  ...
  asyncHandler(async (req, res) => {
    ...
    if (req.query.category) {
      documents = documents.filter(
        (doc) => doc.category === req.query.category
      );
    }
    ...
    res.json({
      ...
      data: {
        ...
        documentsByCategory: {
          receipt: payment.documents.filter((d) => d.category === "receipt")
            .length,
          bank_statement: payment.documents.filter(
            (d) => d.category === "bank_statement"
          ).length,
          cheque_copy: payment.documents.filter(
            (d) => d.category === "cheque_copy"
          ).length,
          upi_screenshot: payment.documents.filter(
            (d) => d.category === "upi_screenshot"
          ).length,
    ...
    res.json({
      ...
      data: {
        ...
        averagePayment:
          completedPayments > 0
            ? Math.round(
                ((totalAmount[0]?.total || 0) / completedPayments) * 100
              ) / 100
            : 0,
        paymentsByMethod,
