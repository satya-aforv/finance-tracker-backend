...
export const createPlan = async (req, res) => {
  ...
  if (planData.paymentType === "interest") {
    if (!planData.interestPayment) {
      return res.status(400).json({
        message:
          "Interest payment configuration is required for interest payment type",
      });
    }
...
router.post(
  ...
  [
    ...
    body("interestType")
      .isIn(["flat", "reducing"])
      .withMessage("Interest type must be flat or reducing"),
    body("interestRate")
      .isFloat({ min: 0, max: 100 })
      .withMessage("Interest rate must be between 0 and 100"),
    body("minInvestment")
      .isFloat({ min: 1000 })
      .withMessage("Minimum investment must be at least 1000"),
    body("maxInvestment")
      .isFloat({ min: 1000 })
      .withMessage("Maximum investment must be at least 1000"),
    body("tenure")
      .isInt({ min: 1, max: 240 })
      .withMessage("Tenure must be between 1 and 240 months"),
    body("paymentType")
      .isIn(["interest", "interestWithPrincipal"])
      .withMessage("Invalid payment type"),
    body("features").optional().isArray(),
    body("riskLevel").optional().isIn(["low", "medium", "high"]),

    // Interest payment validation
    body("interestPayment.dateOfInvestment")
      .optional()
      .isISO8601()
      .withMessage("Invalid date"),
    body("interestPayment.amountInvested").optional().isFloat({ min: 0 }),
    body("interestPayment.interestFrequency")
      .optional()
      .isIn(["monthly", "quarterly", "half-yearly", "yearly", "others"]),
    body("interestPayment.principalRepaymentOption")
      .optional()
      .isIn(["fixed", "flexible"]),
    body("interestPayment.withdrawalAfterPercentage")
      .optional()
      .isFloat({ min: 0, max: 100 }),
    body("interestPayment.principalSettlementTerm")
      .optional()
      .isInt({ min: 1 }),

    // Interest with principal validation
    body("interestWithPrincipalPayment.dateOfInvestment")
      .optional()
      .isISO8601()
      .withMessage("Invalid date"),
    body("interestWithPrincipalPayment.investedAmount")
      .optional()
      .isFloat({ min: 0 }),
    body("interestWithPrincipalPayment.principalRepaymentPercentage")
      .optional()
      .isFloat({ min: 0, max: 100 }),
    body("interestWithPrincipalPayment.paymentFrequency")
      .optional()
      .isIn(["monthly", "quarterly", "half-yearly", "yearly", "others"]),
  ],
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    const hasMajorChanges = majorFields.some(
    ...
    if (activeInvestments > 0 && hasMajorChanges) {
      return res.status(400).json({
        message:
          "Cannot modify core plan parameters when there are active investments. Please create a new plan instead.",
      });
    }
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (
      principalAmount < plan.minInvestment ||
      principalAmount > plan.maxInvestment
    ) {
      return res.status(400).json({
