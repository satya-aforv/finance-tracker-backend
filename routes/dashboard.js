...
router.get(
  ...
  authorize("admin", "finance_manager"),
  asyncHandler(async (req, res) => {
    const { limit = 20, types, userId, startDate, endDate } = req.query;
...
router.get(
  ...
  authorize("admin", "finance_manager"),
  asyncHandler(async (req, res) => {
    const { period = "week", groupBy = "day", types } = req.query;
...
router.get(
  ...
  authorize("admin", "finance_manager"),
  asyncHandler(async (req, res) => {
    const { severity, category, limit = 50, unreadOnly = false } = req.query;
...
router.get(
  ...
  authorize("admin", "finance_manager"),
  asyncHandler(async (req, res) => {
    const { period = "month", compareWith = "previous_period" } = req.query;
...
router.get(
  ...
  authorize("admin", "finance_manager"),
  asyncHandler(async (req, res) => {
    const { benchmarkType = "internal", metrics } = req.query;
...
async function getInvestmentStatsData() {
  ...
  return {
    ...
    averageInvestmentSize:
      totalInvestments > 0 ? (totalValue[0]?.total || 0) / totalInvestments : 0,
  };
}
...
async function getInvestorStatsData() {
  ...
  return {
    ...
    averageInvestment:
      totalInvestors > 0
        ? (totalInvestment[0]?.total || 0) / totalInvestors
        : 0,
    withUserAccounts,
...
async function getPaymentStatsData() {
  ...
  return {
    ...
    averagePayment:
      completedPayments > 0
        ? (totalAmount[0]?.total || 0) / completedPayments
        : 0,
    paymentsByMethod,
  };
}
...
async function getQuickActionsData() {
  try {
    const [overdueCount, pendingKYC, recentInvestments, recentPayments] =
      await Promise.all([
        Investment.aggregate([
          { $unwind: "$schedule" },
          { $match: { "schedule.status": "overdue", status: "active" } },
...
async function getRealTimeMetricsData() {
  try {
    const [activeInvestments, todaysPayments, todaysInvestments] =
      await Promise.all([
        Investment.countDocuments({ status: "active" }),
        Payment.countDocuments({
          paymentDate: {
  ...
  try {
    ...
    if (entities.includes("investors")) {
      const investors = await Investor.find({
        ...
        .limit(limit)
        .select("_id investorId name email phone");
    ...
    if (entities.includes("investments")) {
      const investments = await Investment.find({
        ...
        .select("_id investmentId investor");
    ...
    if (entities.includes("payments")) {
      const payments = await Payment.find({
        $or: [{ paymentId: searchRegex }, { referenceNumber: searchRegex }],
      })
        .limit(limit)
        .populate("investor", "name")
        .select("_id paymentId investor");
