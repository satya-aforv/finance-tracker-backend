...
const RemarksSchema = new mongoose.Schema({
  ...
  userName: String, // 'A. Anjali Mehra'
  userRole: String, // Optional
  content: String, // Main remark text
  date: Date, // When the main remark was created
  attachments: [
    {
      name: String,
      url: String,
...
investmentSchema.methods.addTimelineEntry = function (
  type,
  description,
  performedBy,
  amount = 0,
...
investmentSchema.methods.getDocumentsByCategory = function (category) {
  return this.documents.filter(
    (doc) => doc.category === category && doc.isActive
  );
};
...
investmentSchema.methods.updatePaymentStatus = function () {
  ...
  this.schedule.forEach((payment) => {
    ...
    if (payment.status === "paid") {
      ...
      totalPrincipalPaid += Math.max(
        0,
        payment.paidAmount - payment.interestAmount
      );
