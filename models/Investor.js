...
const investorSchema = new mongoose.Schema(
  {
    ...
    altPhone: {
      type: String,
      required: [false, "Alternate phone number is optional"],
      match: [/^[6-9]\d{9}$/, "Please enter a valid 10-digit phone number"],
    },
    address: {
      present: {
...
investorSchema.methods.getInvestmentSummary = async function () {
  ...
  const summary = {
    ...
    totalExpectedReturns: investments.reduce(
      (sum, inv) => sum + inv.totalExpectedReturns,
      0
    ),
    avgInvestmentSize:
      investments.length > 0
        ? investments.reduce((sum, inv) => sum + inv.principalAmount, 0) /
          investments.length
        : 0,
      ...
      if (
        ...
        overduePayments.push({
          ...
          daysPastDue: Math.ceil(
            (now - scheduleItem.dueDate) / (1000 * 60 * 60 * 24)
          ),
        });
      }
    });
      ...
      if (
        ...
        upcomingPayments.push({
          ...
          daysUntilDue: Math.ceil(
            (scheduleItem.dueDate - now) / (1000 * 60 * 60 * 24)
          ),
        });
      }
    });
...
investorSchema.methods.getPortfolioPerformance = async function () {
  const Investment = mongoose.model("Investment");
  const investments = await Investment.find({ investor: this._id }).populate(
    "plan",
    "name interestRate"
  );
  ...
  investments.forEach((investment) => {
    ...
    performance.investmentBreakdown[planName].totalInvested +=
...
investorSchema.methods.updateStatistics = async function () {
  ...
  this.totalInvestment = investments.reduce(
    (sum, inv) => sum + inv.principalAmount,
    0
  );
  this.activeInvestments = investments.filter(
